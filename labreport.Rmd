...
I was overthinking the approach. So I am simplifing by summarising data first then mutating into the dataframe. 
...
select(well_being_df, Gender, Age) %>%
  group_by(Gender) %>%
  summarise_all(funs(mean, sd, min(.,is.na = FALSE), max(.,is.na = FALSE)))
