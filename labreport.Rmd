...
my.model2 <- lm(formula = pwi_index ~ 
                 affect + 
                 cognition
               )

my.model3 <- lm(formula = pwi_index ~ 
                 affect + 
                 cognition +
                 Open +
                 Consciencious +
...
SWB.Anova <- anova(my.model, my.model2, my.model3) #run an ANOVA 
...
## ~ FUUuuuuuu... bloody doing things the hard way! forgot AGE!
## (╯°□°）╯︵ ┻━┻

### Create a new data frame with age and gender
```{r}
newdf <- cbind.data.frame(well_being_df$Age, well_being_df$Gender, df.all, HPMood, Subjective.Wellbeing)
colnames(newdf)[which(names(newdf) == "well_being_df$Age")] <- "Age"
colnames(newdf)[which(names(newdf) == "well_being_df$Gender")] <- "Gender"
stargazer(newdf, type = "text")
```
