...
Thanks to some Stackexchange help, I can now coerce the dummy codings to their labels. The select() function cannot handle character strings as arguments. For now I have stored the data in the working environment as opposed to mutating it into the dataframe till I am sure I have this correct. 
```{r}
...
well_being_df <- well_being_df %>% 
  mutate(pwi_index = rowSums(select(., contains("PWI")))) #PWI Variable
...
#create a model
my.model <- lm(formula = pwi_index ~ affect + cognition + Personality)
